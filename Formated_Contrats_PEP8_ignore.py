...
def main(wd=None, monitoring=False):
    """
    Función principal para crear el documento de contrato automatizado.

    Args:
        archivo (str): Tipo de archivo a crear ('base' o 'contrato').
        wd (str, optional): Directorio de trabajo donde guardar el archivo.
        monitoring (bool): Indica si se ejecuta dentro del sistema de monitoreo.

    Returns:
        bool: True si el archivo se creó con éxito, False si hubo un error.
    """
    try:
        os.chdir(wd)
        ...
        if wd is None:
            ...
            os.chdir(wd)
            print(f"Directorio {wd} creado porque no existía.")



        print(os.listdir())
        print(os.getcwd())
        print(f"Creando documento de contrato en directorio: {wd}")
        ...
        original_file = "plantilla_original.docx"  # Cambiar el nombre del archivo
        original_path = os.path.join(wd, original_file)
        ...
        if not os.path.exists(original_path):
            print(f"Advertencia: El archivo original {original_path} no se encuentra. Se intentará crear desde cero.")
            word = Document()
        else:
        ...
        portada_template_path = os.path.join(wd, "portada_melipilla_contrato.docx")
        if not os.path.exists(portada_template_path):
            print(f"Error: Plantilla de portada {portada_template_path} no encontrada.")
            return False

        # Generar contextos para renderizar la portada
        context_for_template1, context_for_template2 = generate_contexts(wd)
        if context_for_template2 is None:
            print(f"Error: No se pudo generar contexto para la portada del contrato en {wd}")
            return False

        portada_template = DocxTemplate(portada_template_path)
        portada_template.render(context_for_template2)
        portada_rendered_path = os.path.join(wd, "portada_melipilla_contrato_renderizado.docx")
        portada_template.save(portada_rendered_path)
        doc = Document(portada_rendered_path)
        print("Documento de portada renderizado y cargado.")

        if context_for_template2:
            try:
                # Debo conseguir el nombre del template name
                base_doc = DocxTemplate(original_file)
                base_doc.render(context_for_template2)
                base_doc.save("plantilla_contrato.docx")
        ...
        run1 = heading_paragraph.add_run(
            "CONTRATACION DEL SUMINISTRO DE INSUMOS Y ACCESORIOS PARA TERAPIA DE PRESIÓN NEGATIVA CON EQUIPOS EN COMODATO PARA EL HOSPITAL SAN JOSÉ DE MELIPILLA ENTRE")
        ...
        intro.add_run(
            "En Melipilla, a {{ fecha_contrato }}, entre El Hospital San José de Melipilla, RUT N° 61.602.123-0, representado por don Óscar Vargas Duranti, Chileno, RUT Nº 8.578.583-4, de profesión Médico Cirujano, domiciliado para estos efectos en calle O’Higgins Nº 551, comuna de Melipilla, en adelante “El Hospital” y, por otra parte como proveedor ")
        intro.add_run("“{{ nombre_proveedor }}”").bold = True
        intro.add_run(" en adelante")
        intro.add_run("“el proveedor adjudicado”").bold = True
        intro.add_run(
            ", RUT {{ rut_proveedor }}, representado legalmente por {{ representante_legal }}, cedula nacional de identidad N° {{ rut_representante_legal }}, con domicilio para estos efectos en {{domicilio_representante_legal}}, viene a suscribir el siguiente contrato:")

        doc.add_heading("Primero: Declaraciones.", level=2)
        parrafo_primero = doc.add_paragraph()
        parrafo_primero.add_run(
            "Se deja constancia que el presente servicio ha sido sometido a licitación pública a través del Sistema de Información de la Dirección de Compras y contratación pública (en adelante, “Portal Mercado Público), según ")
        parrafo_primero.add_run("ID {{ id_licitacion }} ").bold = True
        parrafo_primero.add_run(
            "aprobándose las bases de licitación mediante la Resolución Exenta Nº {{ numero_resolucion_aprobacion }} de fecha {{ fecha_resolucion_aprobacion }} y adjudicándose al proveedor ")
        parrafo_primero.add_run("“{{ nombre_proveedor }}”").bold = True
        parrafo_primero.add_run(
            "en virtud de resolución exenta N°{{ resolucion_exenta }} de fecha {{ fecha_resolucion_exenta }}.")
        parrafo_primero.add_run(r"\n")
        parrafo_primero.add_run(
            "“El Proveedor/Empresa” declara que cuenta con la capacidad técnica y el personal necesario para la realización de los servicios contratados por el Hospital San José de Melipilla.")

        # Segundo
        doc.add_heading("Segundo: Objeto del contrato y valor de la contratación.", level=2)
        doc.add_paragraph(
            "El presente contrato tiene por objeto la compra de Suministro de Insumos y Accesorios para Terapia de Presión Negativa con Equipos en Comodato para el Hospital San José de Melipilla, a fin de entregar una prestación de salud integral y oportuna a los usuarios del Hospital y de esta manera dar cumplimiento con el tratamiento de los pacientes.",
            style="List Bullet")
        doc.add_paragraph(
            "La adjudicación se realizará a valor unitario por hora, generando un contrato por $350.000.000.- (Impuestos incluidos.",
            style="List Bullet")
        ...
        contrato_partiendo_desde_portada = Document("contrato_automatizado_over.docx")
        # Extraer y copiar cada sección
        # Plantilla de contrato
        doc_plantillas = Document("plantilla_contrato.docx")
        for seccion in secciones:
            print(f"Procesando sección {seccion}...")
            resultado = extraer_seccion_completa(doc_plantillas, seccion)
            if resultado:
                ...
                copiar_seccion_completa(contrato_partiendo_desde_portada, encabezado, elementos, 2)
                print(f"Sección {seccion} copiada como Nivel 2.")
            else:
                print(f"Sección {seccion} no encontrada.")

        contrato_partiendo_desde_portada.save("contrato_faltan_tablas.docx")
        aplicar_formato_global(doc_plantillas)
        output_path_v2 = os.path.join(wd, "contrato_faltan_tablas.docx")
        ...
        print(f"Error al crear el documento de contrato en {wd}: {e}")
        return False
